...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < bunchSize; i++)
    {
        ...
        {
            qInfo() << "pair (" << (j-1)*bunchSize + i << "," << j*bunchSize + i << "):" << bunchOfImages[j-1].at(i) << "-" << bunchOfImages[j].at(i);
        }
    }
    for(int j = 0; j < bunchOfImages.size(); ++j)
    {
        ...
        {
            qInfo() << "pair (" << j*bunchSize + i - 1 << "," << j*bunchSize + i << "):" << bunchOfImages[j].at(i - 1) << "-" << bunchOfImages[j].at(i);
        }
    }
