...
path: MastactvaShaders/MoveImages/Vertex/mix_varying_t3_1st_pxor_2nd.fsh */
...
void main(void)
{
    ...
    mediump vec3 vt = clamp(v_t, 0.0, 1.0);
    if(vt.x > 0.0 && vt.x < 1.0 || vt.y > 0.0 && vt.y < 1.0 || vt.z > 0.0 && vt.z < 1.0)
    {
        gl_FragColor = mix(s1, vec4(1.0 - s2.r, 1.0 - s2.g, 1.0 - s2.b, s2.a ), max(vt.x, max(vt.y, vt.z))) * opacityArg;
    }
